...
function addToCartFetch(){
  fetch('https://shark-tank-json-server-1.onrender.com/cartpage').then((res)=>res.json()).then((data)=>{
    addCardList(data)
    grandTotal(data)
...
function deleteCartProduct(id){
  fetch(`https://shark-tank-json-server-1.onrender.com/cartpage/${id}`,{
    method: "DELETE"
  })
}
...
function  decreAmount(id){
  fetch(`https://shark-tank-json-server-1.onrender.com/cartpage/${id}`).then((res)=>res.json()).then((data)=>{
